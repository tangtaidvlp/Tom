...
class FlashcardRepository  (database : MemoLangSqliteDataBase){
    ...
    fun deleteCards (flashcard : ArrayList<Flashcard>) {
        val flashcardEntityArray = Array(flashcard.size) {index ->
            return@Array FlashcardConverter.toCardEntity(flashcard[index])
        }
    ...
    private inner class DeleteFlashcardsTask : AsyncTask<Array<FlashCardEntity>, Unit, Unit>() {

        override fun doInBackground(vararg params: Array<FlashCardEntity>?) {
